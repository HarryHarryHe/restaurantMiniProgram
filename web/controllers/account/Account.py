...
def index():
    ...
    page = int(req['p']) if ('p' in req and req['p']) else 1  # 存在并且有值
    query = User.query
    # query.count()  # 总页数

    if 'mix_kw' in req:
        rule = or_(User.nickname.ilike("%{0}%".format(req['mix_kw'])),
                   User.mobile.ilike("%{0}%".format(req['mix_kw'])))  # and, or查询
...
def info():
    resp_data = {}
    req = request.args  # request.args获取GET请求的参数
    uid = int(req.get('id', 0))  # 默认值设置为0？
